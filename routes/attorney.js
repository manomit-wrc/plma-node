...
//fetch
router.get('/attorneys',auth, firmAttrAuth, csrfProtection, (req,res) => {
	var success_message = req.flash('success-message')[0];

	User.findAll({
		where: {role_id : 3}
	}).then(row => {
	res.render('attorney/index',{ layout: 'dashboard', csrfToken: req.csrfToken(),row: row });
});
...
router.get('/attorney/addAttorney',auth, firmAttrAuth, csrfProtection, async (req,res) => {
  ...
  const state = await State.findAll({
	...
	res.render('attorney/addattorney',{ layout: 'dashboard', csrfToken: req.csrfToken(),country: country, state: state, designation: designation,group: group,section: section,jurisdiction: jurisdiction,industry_type: industry_type });
				});
...
router.post('/attorney/add',auth, firmAttrAuth,csrfProtection, async (req,res) => {
	...
	const avatar = gravatar.url(req.body.email, {
		...
		if (attorney_data != null) {
			req.flash('success-message', 'already have the same mail_id');
...
router.get('/attorney/edit/:id',auth,csrfProtection, async (req,res) => {
  ...
  User.hasMany(Attorney_Details, {foreignKey: 'user_id'});
	...
	const state = await State.findAll({});
	 ...
	 const edata = await User.findAll({
		...
		res.render('attorney/updateattorney',{ layout: 'dashboard', csrfToken: req.csrfToken(),country: country, state: state, designation: designation,group: group,section: section,jurisdiction: jurisdiction,industry_type: industry_type,edata: edata });
});
			...
			//
				...
				router.post('/attorney/update/:id',auth, firmAttrAuth,csrfProtection,(req,res) => {
			var attrorney_details_id = '';
		 ...
		 attrorney_details_id = response.id;


			Attorney_Details.update({
					 ...
					 },{where: {user_id : attrorney_details_id}});
